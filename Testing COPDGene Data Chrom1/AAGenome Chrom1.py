...
dataAA=pd.read_csv('/home/jananin/UQ/AA1/AAfactor.txt', sep="\t")

IID=dataAA['IID'].values

AAdf=df[df.columns[df.columns.isin(IID)]]
AAdf.insert(loc=0, column='snp', value=SNP['snp'])

dataAA = dataAA[dataAA.IID.isin(AAdf.columns.values)]
dataAA=dataAA.reset_index(drop=True)

Y=dataAA["factor1_scores"]
X=dataAA[['age_enroll','gender','SmokCigNow','pc1', 'pc2', 'pc3', 'pc4','pc5']]

X = sm.add_constant(X)
est = sm.OLS(Y, X)
est2 = est.fit()

dataAA["Emphysema-PredominantAxis"]=est2.resid

Y=dataAA["factor2scoresflipped"]
X=dataAA[['age_enroll','gender','SmokCigNow','pc1', 'pc2', 'pc3', 'pc4','pc5']]
...
dataAA["Airway-Predominant Axis"]=est2.resid


dataAA.to_csv('dataAA.csv')


AAdf=AAdf.T
AAdf.columns = AAdf.iloc[0]
AAdf= AAdf[1:]
AAdf.index.name = None
AAdf=AAdf.reset_index(drop=True)
#AAdf=AAdf.iloc[: , 1:]


N=np.zeros((3,AAdf.shape[1]))

for j in range(AAdf.shape[1]):
    for i in range(AAdf.shape[0]):
        if AAdf.iloc[i,j]==0 :
            N[0,j]+= 1
        elif AAdf.iloc[i,j]==1:
            N[1,j] += 1
        else:
            N[2,j]+= 1

dropN0=np.where( N[0,:] < .01*(AAdf.shape[0]))
AAdf.drop(AAdf.columns[dropN0],axis=1,inplace=True)
...
dropN1=np.where( N[1,:] < .01*(AAdf.shape[0]))
AAdf.drop(AAdf.columns[dropN1],axis=1,inplace=True)


MAF=np.zeros(AAdf.shape[1])

for j in range(AAdf.shape[1]):
    MAF[j]=(2*N[0,j]+N[1,j])/(2*AAdf.shape[0])

np.savetxt('MAF1.txt',MAF,fmt='%1.2f')

dropSNP=np.where( MAF < 0.05)
AAdf.drop(AAdf.columns[dropSNP],axis=1,inplace=True)

AAdf.to_csv('AAdf.csv')
...
y=dataAA["Emphysema-PredominantAxis"].values

for j in range(nIter):
    Xs[:,j]=(AAdf.iloc[:,j].values)
...
for j in range(nIter):
            ...
            r[0:N[0,j],j]=np.divide(kde_y_obs0(y_initi[0:N[0,j],j]),kde_y_initi0(y_initi[0:N[0,j],j]))
...
samples_to_keepSize=samples_to_keepSize.astype(int)
...
samples_to_keep=samples_to_keep.astype(int)    
...
for j in range(nIter):
    ...
    updated_Beta[samples_to_keepSize[0,j]+samples_to_keepSize[1,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j]+samples_to_keepSize[2,j],j] =Beta[samples_to_keep[
        samples_to_keepSize[0,j]+samples_to_keepSize[1,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j]+samples_to_keepSize[2,j],j],j]

    updated_y[0:samples_to_keepSize[0,j],j] =y_initi[samples_to_keep[0:samples_to_keepSize[0,j],j],j]
    updated_y[samples_to_keepSize[0,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j],j] =y_initi[samples_to_keep[
        samples_to_keepSize[0,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j],j],j]
    updated_y[samples_to_keepSize[0,j]+samples_to_keepSize[1,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j]+samples_to_keepSize[2,j],j] =y_initi[samples_to_keep[
        samples_to_keepSize[0,j]+samples_to_keepSize[1,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j]+samples_to_keepSize[2,j],j],j]
    ...
    updated_eps[samples_to_keepSize[0,j]+samples_to_keepSize[1,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j]+samples_to_keepSize[2,j],j] =eps[samples_to_keep[
        samples_to_keepSize[0,j]+samples_to_keepSize[1,j]:samples_to_keepSize[0,j]+samples_to_keepSize[1,j]+samples_to_keepSize[2,j],j],j]
...
np.savetxt('KruskalPvalueE.txt',KruskalPvalue,fmt='%1.4e')
...
np.savetxt('mannwhitneyPvalueUpB0_B1E.txt',mannwhitneyPvalueUpB0_B1,fmt='%1.4e')
np.savetxt('mannwhitneyPvalueUpB1_B2E.txt',mannwhitneyPvalueUpB1_B2,fmt='%1.4e')
np.savetxt('mannwhitneyPvalueUpB0_B2E.txt',mannwhitneyPvalueUpB0_B2,fmt='%1.4e')
...
Xs=np.zeros((nSamp,nIter))
y=dataAA["Airway-Predominant Axis"].values
...
np.savetxt('KruskalPvalueA.txt',KruskalPvalue,fmt='%1.4e')
...
np.savetxt('mannwhitneyPvalueUpB0_B1A.txt',mannwhitneyPvalueUpB0_B1,fmt='%1.4e')
np.savetxt('mannwhitneyPvalueUpB1_B2A.txt',mannwhitneyPvalueUpB1_B2,fmt='%1.4e')
np.savetxt('mannwhitneyPvalueUpB0_B2A.txt',mannwhitneyPvalueUpB0_B2,fmt='%1.4e')
